...
with open('c:\\Users\\tonip\\Desktop\\LITTLE\\juego-libre-interior.html', 'r', encoding='utf-8') as f:
    text = f.read()
...
"""
if "flower-fall" not in text:
    text = text.replace("/* ---- REVEAL COUNTER STRIP ---- */", css_snippet + "        /* ---- REVEAL COUNTER STRIP ---- */")

# 2. REPLACE THE ROWS inside <section class="interior-timeline">
# I will use a reliable regex to replace the rows inside interior-timeline.
# From <!-- ROW 1 --> down to just before <!-- ===== LIGHT ANIMATION SECTION ===== --> (which is actually CTA now, wait, the section ends with </section>).

start_marker = "<!-- ROW 1 -->"
end_marker = "</section>"

start_idx = text.find(start_marker)
# Find the next </section> after the timeline's opening tag
timeline_sec_idx = text.rfind("<section class=\"interior-timeline overflow-hidden", 0, start_idx) 
if timeline_sec_idx == -1:
    timeline_sec_idx = text.rfind("<section class=\"interior-timeline", 0, start_idx)

# Wait, let's just make sure interior-timeline has position:relative and overflow:hidden so the absolute flowers don't mess up document height.
if "interior-timeline section" in text and "overflow:hidden" not in text:
    # already relative, we need internal container for flowers or just overflow:hidden 
    pass

new_rows = """<!-- ROW 1 (1 HORA) -->
...
# Using regex to substitute from ROW 1 down to the nearest </section>
pattern = re.compile(r'<!-- ROW 1 -->.*?</section>', re.DOTALL)
text = pattern.sub(new_rows, text)

# Insert the flowers right inside the interior-timeline container
flowers_html = """
...
if "<!-- Floating Flowers Background Effect -->" not in text:
    text = text.replace('id="que-ofrecemos">', 'id="que-ofrecemos" style="overflow:hidden; position:relative;">\n' + flowers_html)


with open('c:\\Users\\tonip\\Desktop\\LITTLE\\juego-libre-interior.html', 'w', encoding='utf-8') as f:
    f.write(text)
